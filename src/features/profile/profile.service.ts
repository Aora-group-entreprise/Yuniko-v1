...
async function getPublicProfileRecord(filter: { username?: string; id?: string }): Promise<PublicProfile> {
  ...
  if (postIds.length > 0) {
    const { data: media, error: mediaError } = await db.from("post_media")
      .select("post_id,url,position")
      .in("post_id", postIds)
      .order("position", { ascending: true });
    if (mediaError) throw mediaError;
    for (const row of media ?? []) if (!mediaByPost.has(row.post_id)) mediaByPost.set(row.post_id, row.url);
  }
  ...
  return publicProfileSchema.parse({
    ...
    followStatus,
    posts: (posts ?? [])
      .map((post) => ({ id: post.id, mediaUrl: mediaByPost.get(post.id), caption: post.caption ?? "", createdAt: post.created_at }))
...
export async function updateMyProfile(input: ProfileUpdateInput): Promise<MyProfile> {
  ...
  const { data, error } = await requireYunikoDb().from("profiles")
    .update({
      ...
      is_private: parsed.isPrivate ?? false,
    })
    .eq("id", user.id)
    .select("id,username,display_name,bio,avatar_url,banner_url,is_private,country_code,website,follower_count,following_count")
    .single();
...
export async function uploadMyAvatar(file: File): Promise<MyProfile> {
  if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) throw new Error("Avatar must be a JPEG, PNG, or WebP image.");
