...
const COHORT_KEY = "yuniko.world-cohort.v1";
...
export function getWorldCohort(): string[] {
  ...
  try {
    const raw = JSON.parse(window.localStorage.getItem(COHORT_KEY) ?? "[]");
...
/** Phase 6: sequential global distribution with a separate view window per stage. */
export function getPostDistributionStage(postId: string): DistributionStage {
  ...
  if (cohortStrength(postId, activeCountries) < DISTRIBUTION_RULES.strongAudienceRate) return current;

  const promoted = nextStage(current);
  state[postId] = { stage: promoted, viewsAtStageStart: totalViews };
  writeState(state);
  return promoted;
}
