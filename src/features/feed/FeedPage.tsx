...
export function FeedPage() {
  ...
  return <Suspense fallback={<PageLoadingFallback />}>
    ...
    {view === "search" && <SearchPage onBack={() => setView("feed")} onOpenPost={(postId) => { setSelectedPostId(postId); setView("post"); }} onOpenProfile={() => setView("profile")} />}
...
function FollowingFeed({ onOpenProfile, onOpenCreate, onOpenNotifications, onOpenSearch, onOpenMessages, onOpenSecurity, onOpenSettings, onOpenStory, onCreateStory }: { onOpenProfile: () => void; onOpenCreate: () => void; onOpenNotifications: () => void; onOpenSearch: () => void; onOpenMessages: () => void; onOpenSecurity: () => void; onOpenSettings: () => void; onOpenStory: (storyId: string) => void; onCreateStory: () => void }) {
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ["feed", "world", "personalized"], queryFn: async () => getAlgorithmicFeed(await getWorldFeed()), staleTime: 30_000 });
  useEffect(() => {
    const on = () => setOnline(true); const off = () => setOnline(false); const refreshNotifications = () => setUnreadNotifications(getUnreadNotificationCount());
  ...
  const handlePostSeen = (postId: string) => { if (seenPostsRef.current.has(postId)) return; seenPostsRef.current.add(postId); markPostSeen(postId); };
  return <main className="feed-shell">
    <header className="feed-header">
      ...
      <div className="feed-header-actions"><button type="button" aria-label="Search" onClick={onOpenSearch}><Search size={20} /></button><button type="button" aria-label="Add friends" onClick={onOpenSearch}><UserPlus size={20} /></button><button type="button" aria-label="Security" onClick={onOpenSecurity}><ShieldCheck size={20} /></button><button type="button" aria-label="Settings" onClick={onOpenSettings}><Settings size={20} /></button></div>
    </header>
    ...
    <section className="feed-viewport" data-testid="posts-feed" aria-label="World Feed">{isLoading && <FeedSkeleton />}{isError && <div className="feed-state">Unable to load the feed.</div>}{!isLoading && !isError && data?.posts.map((post) => <div key={post.id} className="feed-slide" onFocus={() => handlePostSeen(post.id)}><PostCard post={post} /></div>)}</section>
    <nav className="bottom-nav" aria-label="Primary navigation"><NavItem label="Home" active><span>⌂</span></NavItem><button type="button" className="nav-item notification-nav" aria-label="Notifications" onClick={onOpenNotifications}><span><Bell size={21} />{unreadNotifications > 0 && <b>{unreadNotifications > 99 ? "99+" : unreadNotifications}</b>}</span><small>Alerts</small></button><button className="create-button" type="button" aria-label="Create" onClick={onOpenCreate}><Plus size={28} /></button><button type="button" className="nav-item" aria-label="Messages" onClick={onOpenMessages}><span><MessageCircle size={21} /></span><small>Messages</small></button><button type="button" className="nav-item" aria-label="Profile" onClick={onOpenProfile}><span><UserRound size={21} /></span><small>Profile</small></button></nav>
  </main>;
}
