...
function FollowingFeed({ onOpenProfile, onOpenCreate, onOpenNotifications, onOpenSearch, onOpenMessages, onOpenSecurity, onOpenSettings, onOpenStory, onCreateStory }: {
  ...
  const markPostSeen = (postId: string) => {
    if (seenBatchRef.current.has(postId)) return;
    seenBatchRef.current.add(postId);
    void import("./feed.service").then(({ markPostsSeen }) => markPostsSeen([postId]).catch(() => undefined));
  };
  ...
  return <main className="feed-shell">
    ...
    <section className="feed-viewport" data-testid="posts-feed" aria-label="World Feed">
      ...
      {!isLoading && !isError && posts.map(post => <div key={post.id} className="feed-slide" onFocus={() => markPostSeen(post.id)}><PostCard post={post} /></div>)}
