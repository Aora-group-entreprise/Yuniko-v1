...
export async function searchYuniko(query: string, limit = 20): Promise<SearchResult> {
  ...
  const [byUsername, byDisplayName, posts] = await Promise.all([
    ...
    db.from("posts").select("id,author_id,caption,created_at").eq("status","ready").is("deleted_at",null)
      .ilike("caption", pattern).order("created_at",{ascending:false}).limit(safeLimit),
  ]);
  ...
  for (const row of [...(byUsername.data ?? []), ...(byDisplayName.data ?? [])]) {
    if (!profileMap.has(row.id)) profileMap.set(row.id, {
  ...
  return {
    ...
    posts: (posts.data ?? []).map(row => ({
      id: row.id, authorId: row.author_id, caption: row.caption ?? "", createdAt: row.created_at,
    })),
  };
}
