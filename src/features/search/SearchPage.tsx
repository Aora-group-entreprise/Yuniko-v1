...
export function SearchPage({ onBack, onOpenPost }: { onBack: () => void; onOpenPost: (postId: string) => void }) {
  ...
  return (
    <main className="feed-shell">
      ...
      <section className="search-page">
        ...
        {!!profiles.length && <section><h2>People</h2><div className="search-profiles">{profiles.map((profile) => <button type="button" className="search-profile" key={profile.id}><img src={profile.avatarUrl} alt="" /><span><strong>{profile.displayName}</strong><small>@{profile.username}</small></span><UserRound size={17} /></button>)}</div></section>}
